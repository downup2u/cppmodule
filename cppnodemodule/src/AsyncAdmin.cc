...
void AsyncWork(uv_work_t *req) {
	...
	asyncData->bSuccess = DataWrapAdmin::get_mutable_instance().getDataMsg(asyncData->sInTypeName, asyncData->sInHexData,
		asyncData->sSessionHexData, asyncData->sOutTypeName, asyncData->sOutHexData, asyncData->isHex);
}
...
{
	return getDataWithSessionAsync(args,true);
}

Handle<Value> getDataWithSessionJsonAsync(const Arguments& args)
{
	return getDataWithSessionAsync(args, false);
